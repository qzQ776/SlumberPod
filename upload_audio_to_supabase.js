...
// 音频文件路径
const audioFilePath = path.join(__dirname, '音频', '虫鸣鸟叫声；森林环境音；树林环境音；田野环境音；深山环境音；_爱给网_aigei_com.mp3');
...
async function uploadAudioAndUpdateDatabase() {
    try {
        ...
        const fileName = '虫鸣鸟叫声；森林环境音；树林环境音；田野环境音；深山环境音；_爱给网_aigei_com.mp3';
        ...
        // 3. 更新MySQL数据库中的audio_id为5的记录
        const connection = await getMysqlConnection();
        
        try {
            // 先检查audio_id为5的记录是否存在
            ...
            if (rows.length === 0) {
                throw new Error('audio_id为5的音频记录不存在');
            }
            ...
            if (updateResult.affectedRows === 1) {
                console.log('✅ 数据库更新成功');
                console.log('✅ 音频URL已更新到audio_id为5的记录中');
